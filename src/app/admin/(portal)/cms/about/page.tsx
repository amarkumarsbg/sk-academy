...
export default function CmsAboutPage() {
  ...
  return (
    <CmsPageShell title="About Page CMS" onSave={save}>
      <SectionCard title="Page Hero">
        <Field label="Title" value={about.hero.title} onChange={(v) => setAbout({ ...about, hero: { ...about.hero, title: v } })} />
        <TextAreaField label="Description" value={about.hero.description} onChange={(v) => setAbout({ ...about, hero: { ...about.hero, description: v } })} />
      </SectionCard>
      ...
      <SectionCard title="Our Story">
        ...
        {about.storyParagraphs.map((para, i) => (
          <ListItemCard key={i} title={`Paragraph ${i + 1}`} onRemove={() => setAbout({ ...about, storyParagraphs: about.storyParagraphs.filter((_, idx) => idx !== i) })}>
            <TextAreaField label="Text" value={para} onChange={(v) => setAbout({ ...about, storyParagraphs: about.storyParagraphs.map((p, idx) => (idx === i ? v : p)) })} />
          </ListItemCard>
        ))}
      ...
      <SectionCard title="Mission & Vision">
        <TextAreaField label="Mission" value={about.mission} onChange={(v) => setAbout({ ...about, mission: v })} />
        <TextAreaField label="Vision" value={about.vision} onChange={(v) => setAbout({ ...about, vision: v })} />
      </SectionCard>

      <SectionCard title="Core Values">
        {about.values.map((value, i) => (
          <ListItemCard key={i} title={`Value ${i + 1}`} onRemove={() => setAbout({ ...about, values: about.values.filter((_, idx) => idx !== i) })}>
            <Field label="Title" value={value.title} onChange={(v) => setAbout({ ...about, values: about.values.map((val, idx) => (idx === i ? { ...val, title: v } : val)) })} />
            <TextAreaField label="Description" value={value.description} onChange={(v) => setAbout({ ...about, values: about.values.map((val, idx) => (idx === i ? { ...val, description: v } : val)) })} />
          </ListItemCard>
        ))}
      ...
      <SectionCard title="Leadership">
        {about.leadership.map((person, i) => (
          <ListItemCard key={i} title={person.name || `Leader ${i + 1}`} onRemove={() => setAbout({ ...about, leadership: about.leadership.filter((_, idx) => idx !== i) })}>
            ...
            <TextAreaField label="Bio" value={person.bio} onChange={(v) => setAbout({ ...about, leadership: about.leadership.map((p, idx) => (idx === i ? { ...p, bio: v } : p)) })} />
          </ListItemCard>
        ))}
