...
export default function CmsAdmissionsPage() {
  ...
  return (
    <CmsPageShell title="Admissions Page CMS" onSave={save}>
      <SectionCard title="Page Hero">
        ...
        <TextAreaField label="Description" value={admissions.hero.description} onChange={(v) => setAdmissions({ ...admissions, hero: { ...admissions.hero, description: v } })} />
      </SectionCard>

      <SectionCard title="Admission Steps">
        {admissions.steps.map((step, i) => (
          <ListItemCard key={i} title={`Step ${step.step}`} onRemove={() => setAdmissions({ ...admissions, steps: admissions.steps.filter((_, idx) => idx !== i).map((s, idx) => ({ ...s, step: idx + 1 })) })}>
            ...
            <TextAreaField label="Description" value={step.description} onChange={(v) => setAdmissions({ ...admissions, steps: admissions.steps.map((s, idx) => (idx === i ? { ...s, description: v } : s)) })} />
          </ListItemCard>
        ))}
      ...
      <SectionCard title="Eligibility & Documents">
        {admissions.eligibilityItems.map((item, i) => (
          <ListItemCard key={i} title={`Item ${i + 1}`} onRemove={() => setAdmissions({ ...admissions, eligibilityItems: admissions.eligibilityItems.filter((_, idx) => idx !== i) })}>
      ...
      <SectionCard title="Important Dates">
        {admissions.importantDates.map((item, i) => (
          <ListItemCard key={i} title={item.label || `Date ${i + 1}`} onRemove={() => setAdmissions({ ...admissions, importantDates: admissions.importantDates.filter((_, idx) => idx !== i) })}>
